...
const ScoreQuiz = () => {
  ...
  const [detectedLocation] = useState("Montréal, QC");

  const questions = [
    // Partie 1: Contact & Contexte (Q1-Q4)
    {
      id: 'name',
      type: 'text',
    ...
    },
    {
      ...
      placeholder: '(optionnel) pour démo prioritaire si score critique',
      required: false,
      progress: '3/15'
    },
    {
      id: 'location',
      type: 'text',
      question: 'ta localisation',
      placeholder: '',
      required: false,
      autoDetected: detectedLocation,
      progress: '4/15'
    },
    // Q5: Taille Entreprise
    {
      id: 'companySize',
      type: 'radio',
      ...
      required: true,
      insight: 'nous adaptons les recommandations selon ta taille',
      progress: '5/15'
    },
    // Partie 2: Best Practices (Q6-Q13)
    {
      id: 'centralization',
      type: 'radio',
    ...
    // Partie 3: Qualification Business (Q14-Q18)
    {
      id: 'objective',
      type: 'radio',
  ...
  const calculateScore = () => {
    ...
    setTimeout(() => {
      ...
      // Redirect to results
      window.location.href = `/results?score=${scoreFinal}&category=${category}&answers=${encodeURIComponent(JSON.stringify(answers))}`;
    }, 2000);
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary-600 to-primary-800 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="text-primary-200">nous calculons ton score de chaos administratif</p>
        </div>
      </div>
    );
  }
    ...
    <section className="min-h-screen bg-gradient-to-br from-neutral-50 to-white py-12">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className={`bg-white rounded-2xl shadow-xl p-8 border border-neutral-200 transition-all duration-500 transform ${
          ...
          {/* Auto-detected info */}
          {currentQ.autoDetected && (
            <div className="bg-primary-50 border border-primary-200 rounded-lg p-3 mb-6">
              <p className="text-sm text-primary-700">
                📍 Localisation détectée : {currentQ.autoDetected}
              </p>
            </div>
          )}

          {/* Input fields */}
          {currentQ.type === 'text' && !currentQ.autoDetected && (
            <input
              type="text"
          ...
          {showInsight && currentQ.insight && (
            <div className="mt-4 p-4 bg-accent-50 border border-accent-200 rounded-lg">
              <div className="flex items-start space-x-2">
                ...
                <p className="text-sm text-accent-800">{currentQ.insight}</p>
              </div>
            </div>
          )}

          {/* Info bubble */}
          {currentQ.insight && !showInsight && currentQ.type === 'radio' && (
